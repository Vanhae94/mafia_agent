...
2.  **Discussion Loop**:
    *   `select_next_speaker`: 대화 맥락을 분석해 다음 화자 선정 (직전 발언자 제외하여 다양한 캐릭터 참여 유도).
    *   `character_speak`: 선정된 캐릭터가 LLM을 통해 발언 생성 (의심 수치에 따라 감정적 반응 변화).
    *   `wait_user`: 사용자의 개입(끼어들기, 의심하기, 투표 등)을 대기.
3.  **Suspicion**: 사용자가 특정 AI를 의심하면 수치 증가.
4.  **AI Suspicion**: 낮 토론 종료 후, AI들이 서로를 평가하여 의심 수치 업데이트.
5.  **Summarize Round**: 라운드 종료 시 LLM이 대화 내용을 요약하여 다음 라운드에서 캐릭터들이 기억할 수 있도록 함 (토큰 관리 및 장기 기억).
6.  **Night Phase**: 
    *   팬텀이 생존자 중 한 명을 습격(제거).
    *   LLM이 팬텀의 특징(직업, 성격)을 암시하는 현장 단서 생성.
7.  **Vote**: 사용자가 범인을 지목하여 승패 결정.
